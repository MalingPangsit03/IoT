// === Final Version: Dynamic Threshold, Upload-on-Change & Alert Flags ===
#include <WiFi.h>
...
// Thresholds (will be fetched from server)
...
void applyAlerts(float t, float h) {
  ...
  bool alertState = tempAlertActive || humAlertActive;
...
void tryUpload(float temp, float hum) {
  ...
  doc["temp_alert"]  = tempAlertActive ? 1 : 0; // Alert flag
  doc["hum_alert"]   = humAlertActive ? 1 : 0;  // Alert flag
